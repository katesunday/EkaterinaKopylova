...
this.todayWeather = function(pos) {
  ...
  temp.innerHTML = `Температура сейчас равна: <strong>${data.main.temp} &#176;C</strong>, но ощущается как <strong>${data.main.feels_like} &#176;C</strong>.`;
  description.innerHTML = `В целом на улице: <strong>${data.weather[0].description} </strong>, влажность: <strong>${data.main.humidity}%</strong>.`;
